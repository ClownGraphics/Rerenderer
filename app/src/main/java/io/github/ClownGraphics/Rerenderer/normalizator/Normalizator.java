...
public class Normalizator {
    ...
    public ObjFile computeNormals(ObjFile file) {
        ...
        for (Vertex vertex : vertices) {
            ...
            for (int i = 0; i < 3; i++) {
                vectors[i] = new Vec3(vertices.get(vertexIndices.get(i)).x(), vertices.get(vertexIndices.get(i)).y(), vertices.get(vertexIndices.get(i)).z());
            }
        ...
        // Проверить, скорее всего нормали не изменяются, так как меняем не textureVertices
        return new ObjFile(new VertexData(vertices, file.vertexData().textureVertices(), file.vertexData().vertexNormals(), file.vertexData().parameterVertices()), file.elements(), file.groupingData());
    }
