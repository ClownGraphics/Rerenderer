...
public final class Triangulation {
    ...
    // Ориентация (против часовой стрелки/по часовой стрелке) вычисляется с помощью
    // площади многоугольника,
    // которая рассчитывается через сумму попарных произведений координат вершин.
    // Если площадь отрицательная, многоугольник ориентирован против часовой
    ...
    private static <T extends Vector2f> boolean isEar(List<T> vertices, List<Integer> vertexIndices,
            int prevVertexIndex, int curVertexIndex, int nextVertexIndex) {
        Vector2f prevVertex = vertices.get(prevVertexIndex);
        Vector2f curVertex = vertices.get(curVertexIndex);
        Vector2f nextVertex = vertices.get(nextVertexIndex);
